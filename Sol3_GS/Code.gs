...
var startDate = Utilities.formatDate(dateToString(formObject.startDate), "EST", "MMMM dd',' yyyy hh:mm:ss z");
  ...
  var fileid,checkexist=0;
 while (files.hasNext()) {
   var file = files.next();
   if(file.getName()=='GS_events')
   {
     ...
     if(checkexist==0)
   {
   var ssNew = SpreadsheetApp.create("GS_events");
...
for (var i=0;i<events.length;i++) {
  ...
  Logger.log("Events generated by"+Session.getActiveUser().getEmail());
}
  sendReport();
}
...
for (var i=0;i<events.length;i++) {
  ...
  cell.setFormula('=(HOUR(F' +row+ ')+(MINUTE(F' +row+ ')/60))-(HOUR(E' +row+ ')+(MINUTE(E' +row+ ')/60))');
  cell.setNumberFormat('.00');  
  Logger.log("Daily report accessed by"+Session.getActiveUser().getEmail());
}
...
function sendReport() {
 ...
 while (files.hasNext()) {
  ...
  var responses = SpreadsheetApp.openById(fileid).getActiveSheet();
 ...
 // var responses = SpreadsheetApp.openById('1fOKAKmTAYQFP8ESFPAUtFerhXdaOrX4BToxt_Tk5Jss').getActiveSheet();
  ...
  Logger.log("Email delivered to: "+Session.getActiveUser().getEmail());
}
...
function createTimeDrivenTriggers(formObject) {
  ...
  Logger.log("Trigger created"+Session.getActiveUser().getEmail());
}
